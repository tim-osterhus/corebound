...
class IronLanternDescentArcadeReleaseTests(unittest.TestCase):
    def test_manifest_lists_truthful_cinder_vent_network_release_and_local_thumbnail(self) -> None:
        ...
        self.assertIn("draft gates", game["summary"])
        self.assertIn("fresh-air relays", game["summary"])
        self.assertIn("v0.3.0 Cinder Vent Network", game["release"]["label"])
        self.assertIn("draft gates", game["release"]["copy"])
        self.assertIn("filter cartridges", game["release"]["copy"])
        self.assertIn("pressure fans", game["release"]["copy"])
        self.assertIn("route-stability recovery", game["release"]["copy"])
        self.assertNotIn("snapshot", game)
        self.assertEqual("0.3.0", game["versions"][0]["version"])
        self.assertEqual("games/iron-lantern-descent/versions/0.3.0/", game["versions"][0]["path"])
        self.assertEqual("2026-05-01", game["versions"][0]["releasedAt"])
        self.assertEqual("v0.3.0 Cinder Vent Network", game["versions"][0]["label"])
        self.assertIn("cinder vent shaft and fan relay bay passages", game["versions"][0]["summary"])
        self.assertIn("gas-pocket decisions", game["versions"][0]["summary"])
